...
class Bot:
    ...
    def __init__(self):
        ...
        # Get bar size
        self.barsize = int(
            input("Enter the barsize you want to trade in minutes : "))
        mintext = " min"
        if (int(self.barsize) > 1):
            mintext = " mins"
        queryTime = (datetime.now().astimezone(pytz.timezone("America/New_York"))-timedelta(days=1)
                     ).replace(hour=16, minute=0, second=0, microsecond=0).strftime("%Y%m%d %H:%M:%S")
        ...
        self.ib.reqHistoricalData(self.reqId, contract, "", "2 D", str(
            self.barsize)+mintext, "TRADES", 1, 1, True, [])
    ...
    def on_bar_update(self, reqId, bar, realtime):
        ...
        if (realtime == False):
            ...
            bartime = datetime.strptime(bar.date, "%Y%m%d %H:%M:%S").astimezone(
            ...
            if (minutes_diff > 0 and math.floor(minutes_diff) % self.barsize == 0):
                ...
                if (bar.close > lastHigh
                    ...
                    bracket = self.bracketOrder(
                        orderId, "BUY", quantity, profitTarget, stopLoss)
                    contract = Contract()
                    contract.symbol = self.symbol.upper()
                    contract.secType = "STK"
                    contract.exchange = "SMART"
